...
std::filesystem::path filesystem::search_path(const char* name) {
...
ProcessBuilder::ProcessBuilder(const std::filesystem::path& path) : _path{path}, _name_inferred{true} {
...
namespace mem {
    ...
    public:
        ...
        bool empty() const { return _storage == nullptr; }
    ...
    template<class T>
    Shared<T>::Shared() : _storage{nullptr} {
        
    }
    
    template<class T>
    Shared<T>::Shared(const T& value) : Shared{} {
        alloc();
    ...
    template<class T>
    Shared<T>& Shared<T>::operator=(const T& value) {
        if (empty()) {
            alloc();
        } else {
            _storage->~T();
