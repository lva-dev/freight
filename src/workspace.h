...
inline int debuglevel_to_int(DebugInfo level) {
...
public:
    Manifest(TomlManifest&& toml, const std::string& name, std::vector<Target>&& targets)
        : _toml{std::move(toml)}, _name{name}, _targets{std::move(targets)} {}

    const std::string& name() const { return _name; }
    std::span<const Target> targets() const { return _targets; }
    const TomlManifest& toml() { return _toml; }
};
...
public:
    ...
    : _manifest(manifest), _manifest_path{_manifest_path} {}
    
    const std::filesystem::path& manifest_path() const {
        return _manifest_path;
    }

    std::filesystem::path root() const { return manifest_path().parent_path(); }
    const Manifest& manifest() const { return _manifest; }
    const std::string& name() const { return manifest().name(); }
    std::span<const Target> targets() const { return manifest().targets(); }
};
...
public:
    ...
    const std::filesystem::path& cwd() const { return _cwd; }
...
public:
    ...
    std::filesystem::path root() const { return root_manifest().parent_path(); }
...
namespace filesystem {
    bool contains_manifest(const std::filesystem::path& dir);
