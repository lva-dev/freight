...
namespace freight {
	...
	namespace err {
		...
		public:
            ...
            Failure& msg(std::string_view message) {
				io::eprintln("\n{}", message);
				return *this;
			}
			...
			Failure& more_info() {
				io::eprintln(
					"\nFor more information, try '\033[36m--help\033[32m'.");
				return *this;
			}

			Failure& usage(std::string_view usage) {
				io::eprintln(
		...
		inline Failure unexpected_arg(std::string_view arg) {
			io::error(std::format("unexpected argument '{}' found", arg));
			return Failure();
		}
