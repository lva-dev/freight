...
namespace io {
	 ...
	 * Equivalent to `std::print(stderr, fmt, std::forward<Args>(args)...);`.
	...
	/**
	 * @brief Prints a formatted string to stderr.
	 * Equivalent to `std::println(stderr, fmt, std::forward<Args>(args)...);`.
	...
	/**
	 * @brief Prints an error message (terminated with a new line) with in the
	...
	/**
	 * @brief Prints a warning message (terminated with a new line) to stderr.
	 * @param message the message to print
	 */
...
namespace err {
	...
	public:
		...
		~Failure() {
			if (!_called_exit)
				assert("Failure.exit() was not called for this Failure object");
		}
	...
	/**
	 * @brief Exits the program with exit code 1 after printing an error message
	 * (terminated with a new line) to stderr. After printing, invokes
	 * `std::exit(1)`.
	 * @param message the message to print
	 */
